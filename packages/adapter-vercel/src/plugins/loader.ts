  ...
  return [
    ...
    ...targetLoader("vercel", {
      ...
      async load(_id, { meta }) {
        ...
        if (isServerEntry && (!entry.server || (entry.server === "srvx" && !isEdge))) {
          //language=javascript
...
const serverName = fetchable?.server?.name;

const exportDefault = serverName && serverName !== "srvx" ?
    servers[serverName].${fn}(fetchable.server.app) :
